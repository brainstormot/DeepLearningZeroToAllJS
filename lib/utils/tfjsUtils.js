 ...
 *  @return {object} number[x_data.legnth ,1] : predicted label 
 */
async function predictedAsync(predict_Tensor){
    let data = await predict_Tensor.data()
    let predict  = _.chunk(data,nb_classes) // dataSync() and data() returns flatten data.
    let oneHotVectors = _.chain(predict).map(
        function(row){
            let oneHotVector = Array(nb_classes).fill(0)
            oneHotVector[argMax(row)] = 1;
